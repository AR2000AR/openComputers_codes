...
class User(Model):
    ...
    def password(self, value: str):
        self._database.execute(
            "UPDATE users SET password = ? WHERE login = ?", (self.hashpassword(value), self.login))
    ...
    def secureVerify(self, hashed: bytes):
        ...
        hashed2 = hashlib.sha256(
            self.authenticationSalt+self.passwordHash).digest()
    ...
    def subnetid(self):
        cursor = self._database.execute(
            "SELECT subnetid FROM users WHERE login = ?", (self.login,))
        return cursor.fetchone()[0]

    @subnetid.setter
    def subnetid(self, value):
        self._database.execute(
            "UPDATE users SET subnetid = ? WHERE login = ?", (value, self.login))
        self._database.commit()
    ...
    def addNetwork(self, net: Network, perm=0):
        self._database.execute(
            "INSERT INTO networkUsers VALUES(?,?,?)", (net.id, self.login, perm))
        self._database.commit()

    def get_network_permission(self, net: Network) -> int:
        cursor = self._database.execute(
            "SELECT permissions FROM networkUsers WHERE userid = ? AND networkid = ?", (self.login, net.id))
    ...
    def subnetwork_permission(self) -> SubnetworkPermission:
        cursor = self._database.execute(
            "SELECT subnetperm FROM users WHERE login = ?", (self.login,))
        res = cursor.fetchone()
        assert (res)
        return SubnetworkPermission(res[0])

    @subnetwork_permission.setter
    def subnetwork_permission(self, perm: SubnetworkPermission):
        self._database.execute(
            "UPDATE users SET subnetperm = ? WHERE login = ?", (perm.value, self.login))
        self._database.commit()
...
class Users(Model):
    ...
    def get_users_in_subnet(self, subnetid: int):
        cursor = self._database.execute(
            "SELECT login FROM users WHERE subnetid = ?", (subnetid,))
    ...
    def createUser(self, login: str, password: str, subnetid=None):
        if (self.getUserFromLogin(login)):
            ...
            cursor = self._database.execute("INSERT INTO users VALUES(?,?,?,?)", (
                login, User.hashpassword(password), subnetid, int(subnetperm)))
            self._database.commit()
    ...
    def deleteUser(self, user: User):
        ...
        self._database.execute(
            "DELETE FROM users WHERE login = ?", (user.login,))
        if (not self._database.execute("SELECT * FROM users WHERE subnetid = ?", (subnetwork,)).fetchall()):
            self._database.execute(
                "DELETE FROM networks WHERE subnetid = ?", (subnetwork,))
        self._database.commit()
