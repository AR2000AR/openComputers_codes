...
-- Found at http://lua-users.org/wiki/SecureHashAlgorithm
-- Shortened and adapted for OC by AR2000AR
...
local function digestblock(msg, i, H)
	...
	for j = 17, 64 do
		local v = w[j - 15]
		local s0 = bit32.bxor(bit32.rrotate(v, 7), bit32.rrotate(v, 18), rshift(v, 3))
		v = w[j - 2]
		local s1 = bit32.bxor(bit32.rrotate(v, 17), bit32.rrotate(v, 19), rshift(v, 10))
...
---@param data string
---@return string bytes
local function hash256(data)
	data = preproc(data, #data)
	local H = initH256(HH)

	-- Process the message in successive 512-bit (64 bytes) chunks:
	for i = 1, #data, 64 do
		digestblock(data, i, H)
	end
