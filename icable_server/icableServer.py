...
class Node(threading.Thread):
            ...
            while run.is_set():
                ...
                match packet.kind:
                    case ICABLE_PACKET_KIND.CLIENT_AUTH:
                        self.handler_client_auth(IcableClientAuth.unpack(packet.payload))
                    case ICABLE_PACKET_KIND.CLIENT_AUTH_REQUEST:
                        self.handler_client_auth_request(IcableClientAuthRequest.unpack(packet.payload))
                    case _ if (not self.user):
                        self.error(SERVER_ERROR_KIND.NOT_AUTH,'Client is not authenticated')
                    case ICABLE_PACKET_KIND.CLIENT_NETCONF:
                        self.handler_netconf(IcableNetconf.unpack(packet.payload))
    ...
    def error(self,kind:SERVER_ERROR_KIND,msg=None):
        breakpoint()
        self.socket.send(IcablePacket(ICABLE_PACKET_KIND.SERVER_ERROR,IcableServerError(kind,msg).pack()).pack())
    ...
    def handler_client_auth(self,auth:IcableClientAuth):
        ...
        if(not user):
            icablePayload = IcablePacket(ICABLE_PACKET_KIND.SERVER_AUTH,srvAuth.pack())
        ...
        match self._pendingUser:
            ...
            case pendingUser if isinstance(pendingUser,models.user.User): #secure auth
    ...
    def handler_client_auth_request(self,info:IcableClientAuthRequest):
        ...
        if(user):
            ...
            serverAuthRequest = IcableServerAuthRequest(user.salt,user.authenticationSalt)
            icablePacket = IcablePacket(ICABLE_PACKET_KIND.SERVER_AUTH_REQUEST,serverAuthRequest.pack())
            self.socket.send(icablePacket.pack())
        else:
            self.socket.send(IcablePacket(ICABLE_PACKET_KIND.SERVER_AUTH(IcableServerAuth(False))))
    ...
    def handler_netconf(self,netconf:IcableNetconf):
        match netconf.kind:
            case CLIENT_NETCONF_KIND.AUTO_IPv4 if netconf.ipv4:
                ...
                if(not allowed):
                    self.error(SERVER_ERROR_KIND.NETCONF_ERROR,'Network not allowed')
                else:
                    self.error(SERVER_ERROR_KIND.UNKNOWN_MSG,'Automatic address attribution on a network is not implemented')
            case CLIENT_NETCONF_KIND.AUTO_IPv4:
                self.error(SERVER_ERROR_KIND.UNKNOWN_MSG,'Automatic address attribution is not implemented')
            case CLIENT_NETCONF_KIND.MANUAL_IPv4 if netconf.ipv4 not in [client.address[0] for client in clientList]:
                ...
                if(allowed==True):
                    ...
                    icablePayload = IcablePacket(ICABLE_PACKET_KIND.SERVER_NETCONF,netconf.pack())
            ...
            case CLIENT_NETCONF_KIND.MANUAL_IPv4:
                if(netconf.ipv4 in [client.address[0] for client in clientList]):
                    self.error(SERVER_ERROR_KIND.NETCONF_ERROR,'Address already in use')
            case _:
                self.error(SERVER_ERROR_KIND.NETCONF_ERROR,"Unsuported netconf kind")
    ...
    def handler_data(self,packet:IcablePacket):
        ...
        if(ipaddress.ip_address(ipMsg.src) != self.address[0]):
            ...
            return
        try:
            client = [client for client in clientList if client.address[0] == ipaddress.ip_address(ipMsg.dst)][0]
            if(firewalls[self.user.subnetid].getAction(ipMsg)== FIREWALL_ACTION.ALLOW):
                client.socket.send(IcablePacket(ICABLE_PACKET_KIND.SERVER_DATA,ipMsg.pack()).pack())
        except IndexError:
...
class WebInterface(BaseHTTPRequestHandler):
    ...
    def parse_POST(self)->dict[str,list[str]]:
        ...
        postvars = {k.decode():[vv.decode() for vv in v] for k,v in postvars.items()}
        return postvars
    ...
    def get_clients(self):
        ...
        html = self.get_template('clients.html').safe_substitute(clientList=tableContent)
    ...
    def get_networks(self):
        ...
        for user in users.get_users_in_subnet(self.user.subnetid):
            mapping['poolusers']+=f"""<tr>
                <td>{user.login}</td>
                <td>{user.subnetwork_permission.name}</td>
            </tr>
            """
    ...
    def post_network(self):
        ...
        if(self.post['cmd'][0] == 'new'):
            network = ipaddress.ip_network((self.post['network'][0],self.post['mask'][0])) 
    ...
    def post_signup(self):
        if(not users.getUserFromLogin(self.post['username'][0])):
            if(self.post['password'][0] == self.post['passwordConfirmation'][0]):
                user = users.createUser(self.post['username'][0],self.post['password'][0])
                if(user):
                    self.cookies['sid'] = str(webSessions.createSession(user))
...
webserverThread = threading.Thread(target=webserver.serve_forever,name="Web interface")
